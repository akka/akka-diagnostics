...
import com.lightbend.akka.diagnostics.ConfigChecker.ConfigWarning
...
class ConfigCheckerSpec extends AkkaSpec(ConfigCheckerSpec.conf) {
  ...
  private val isAkka26: Boolean =
    system.settings.ConfigVersion.startsWith("2.6")

  private def adjustRemoteConfigPath(path: String): String = {
    if (isAkka26 && path.startsWith("akka.remote.") && !path.startsWith("akka.remote.classic") &&
      !path.startsWith("akka.remote.artery") && !path.startsWith("akka.remote.watch-failure-detector"))
      "akka.remote.classic" + path.substring(11)
    else
      path
  }

  val defaultRemote = ConfigFactory.parseString(s"""
    akka.actor.provider = akka.remote.RemoteActorRefProvider
    # otherwise it will warn about undefined hostname
    akka.remote.artery.canonical.hostname = 127.0.0.1
    ${adjustRemoteConfigPath("akka.remote.netty.tcp.hostname")} = 127.0.0.1
    ${adjustRemoteConfigPath("akka.remote.netty.ssl.hostname")} = 127.0.0.1
    """).withFallback(reference)

  val defaultCluster = ConfigFactory.parseString(s"""
  ...
  "The ConfigChecker" must {
    ...
    "have valid default power user settings" in {
      ...
      checker.powerUserSettings.foreach { p =>
        val path = adjustRemoteConfigPath(p)
        // FIXME remove if when only using AKka 2.6
        if (!path.startsWith("akka.remote.classic")) {
          withClue("wrong path: " + path) {
            reference.hasPath(path) should be(true)
          }
        }
      }
    ...
    "warn for jackson serialization if older than 2.6" in {
      val c = ConfigFactory.parseString("akka.serialization.jackson.type-in-manifest=off")
        .withFallback(reference)
      val checker = new ConfigChecker(extSys, c, reference)

      val warnings = checker.check().warnings
      if (checker.isAkka26) warnings.shouldBe(Seq.empty[ConfigWarning])
      else warnings.length.shouldBe(1)
    }
    ...
    "find typos and misplacements" in {
      ...
      assertPath(warnings, "akka.log-level", "akka.actor.loglevel",
        ...
        "my-dispatcher.throowput", "akka.actor.deployment.foo")
    ...
    "find too many dispatchers" in {
      val c = (1 to 11).map(n => ConfigFactory.parseString(
        s"""
          disp-$n = {
    ...
    "find suspect SBR configuration" in {
      val configStrings = List(
        ...
        s"""
          akka.cluster.auto-down-unreachable-after = 10s
          akka.cluster.split-brain-resolver.active-strategy = keep-majority
          akka.diagnostics.checker.disabled-checks = ${if (isAkka26) "[auto-down, typo]" else "[auto-down]"}
          """)
    ...
    "log warning when ActorSystem startup" in {
      ...
      try {
        EventFilter.warning(start = "Lightbend recommendation", source = logSource, occurrences = 1).intercept {
        }(sys2)
      } finally {
        shutdown(sys2)
      }
    ...
    "not warn about the dynamic hostnames when artery is used" in {
      ...
      val config2 = ConfigFactory.parseString("""akka.remote.artery.canonical.hostname = "<getHostName>" """).withFallback(config1)
    ...
    "not warn about paths with quotes" in {
      val config = ConfigFactory.parseString("""akka.grpc.client."*".ssl-config.hostnameVerifierClass = 37""").withFallback(reference)
