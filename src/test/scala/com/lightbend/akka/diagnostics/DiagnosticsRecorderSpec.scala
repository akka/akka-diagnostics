...
object DiagnosticsRecorderSpec {
  val config = """
    akka.extensions = ["com.lightbend.akka.diagnostics.DiagnosticsExtension"]
  ...
  "DiagnosticsRecorder" must {
    ...
    "write readme" in {
      startNewSystem("Sys3") { (_, file) =>
        ...
        readme should include("http://support.lightbend.com")
    ...
    "not fail the system when directory cannot be created" in {
      val f = new File("akka-diagnostics/target", "akka-diagnostics-file")
      if (f.createNewFile())
        try {
          val sys = ActorSystem(
            "akka-diag-file",
