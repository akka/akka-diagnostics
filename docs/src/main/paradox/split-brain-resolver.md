...
This module has been open sourced. See [Split Brain Resolver](https://doc.akka.io/docs/akka/current/split-brain-resolver.html) in the Akka docs. 
...
the @extref:[recommendation](akka:cluster-usage.html#auto-downing-do-not-use-) is that the decision of what to
...
To use the Split Brain Resolver feature a dependency on the *akka-split-brain-resolver* artifact must be added.
...
"com.lightbend.akka" %% "akka-split-brain-resolver" % "$version$"
```
@@@
...
// Add Lightbend Platform to your build as documented at https://developer.lightbend.com/docs/lightbend-platform/introduction/getting-started/subscription-and-credentials.html
dependencies {
  compile group: 'com.lightbend.akka', name: 'akka-split-brain-resolver_$scala.binaryVersion$', version: '$version$'
}
```
@@@
...
<dependency>
  ...
  <artifactId>akka-split-brain-resolver_$scala.binaryVersion$</artifactId>
...
@@snip [reference.conf](/akka-split-brain-resolver/src/main/resources/reference.conf) { #split-brain-resolver }
...
That is handled by @extref:[Coordinated Shutdown](akka:actors.html#coordinated-shutdown)
...
@extref:[Coordinated Shutdown](akka:actors.html#coordinated-shutdown) (SIGTERM).
...
@@snip [reference.conf](/akka-split-brain-resolver/src/main/resources/reference.conf) { #static-quorum }
...
@@snip [reference.conf](/akka-split-brain-resolver/src/main/resources/reference.conf) { #keep-majority }
...
@@snip [reference.conf](/akka-split-brain-resolver/src/main/resources/reference.conf) { #keep-oldest }
...
@@snip [reference.conf](/akka-split-brain-resolver/src/main/resources/reference.conf) { #keep-referee }
...
The `lease-majority` is currently marked as @extref:[May Change](akka:common/may-change.html) in the sense of
...
in Kubernetes. It is described in the @ref:[Kubernetes Lease](kubernetes-lease.md) documentation.
...
@@snip [reference.conf](/akka-split-brain-resolver/src/main/resources/reference.conf) { #lease-majority }

See also configuration and additional dependency in @ref:[Kubernetes Lease](kubernetes-lease.md)
...
Akka Cluster has @extref:[support for multiple data centers](akka:cluster-dc.html), where the cluster
